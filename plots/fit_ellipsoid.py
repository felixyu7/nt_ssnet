...
def fit_ellipsoid(event, nhit=8):
    if len(event.filtered.t) <= nhit:
        return None
    else:
        from scipy.optimize import differential_evolution

        total_x = event.filtered.sensor_pos_x.to_numpy()
        total_y = event.filtered.sensor_pos_y.to_numpy()
        total_z = event.filtered.sensor_pos_z.to_numpy()
